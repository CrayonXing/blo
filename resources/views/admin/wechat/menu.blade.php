                                ...
                                <div class="ibox-content" style="padding: 15px;min-height: 400px;">
                                    ...
                                    <form method="get" class="form-horizontal">
                                        <div class="form-group">
                                            <label class="col-sm-1 control-label">菜单标题</label>
                                            <div class="col-sm-11">
                                        ...
                                        <div class="cus-menu-box form-group hidden">
                                            <label class="col-sm-1 control-label">菜单内容</label>
                                            <div class="col-sm-11">
                                            ...
                                            <div class="col-sm-offset-1 col-sm-11">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-sm-12">
                                                                <form role="form">
                                                                    <div class="form-group">
                                                                        <label>小程序ID:</label>
                                                                        <input type="text" placeholder="在小程序后台获取" class="form-control">
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label>小程序页面路径:</label>
                                                                        <input type="text" placeholder="小程序页面路径" class="form-control">
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label>页面地址:</label>
                                                                        <input type="text" placeholder="页面地址，当不支持小程序时会跳转此页面" class="form-control">
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="my-box-type3 my-box">
                                                    ...
                                                    <div style="width: 292px;height: 137px;background:white;margin: 0 auto;border: 1px dashed #f1e1e1;">
                                                        <p style="margin: 0;height: 30px;line-height: 30px;background: #ebe7e7;text-align: center">资源名:关于我们</p>
    ...
    <script type="text/javascript">
        ...
        function Menu(menuJsonStr){
            ...
            this.init = function(){
                ...
                if(this.menuJsonStr.button && Array.isArray(this.menuJsonStr.button)){
                    ...
                    $('#menu-list').find('.current').removeClass('current');
                }
            };
            ...
            this.getSubItemHtml = function (ops){
                var ops = ops || {type: "click",name: "添加子菜单",key: "key|"};

                return `<li class="sub-menu-item current" data-type="${ops.type}" data-action="${ops.key}" data-name="${ops.name}" data-url="${ops.url}"><a href="javascript:;"><span class=""><i class="weixin-icon sort-gray"></i><span class="sub-title">${ops.name}</span></span></a></li>`;
            };

            /**
             * 设置编辑区域
             */
            this.setEditArea = function (type,data,el){
                this.currentEdit = {type,data};
                if(type == 'sub-menu-item'){
                    $("input[type=radio][name=type][value='" + el.data('type') + "']").prop("checked", "checked").trigger('click');
                    $('.cus-menu-box').removeClass('hidden');
                }else{
                    if(data.subMenuNum > 0){
                        $('.cus-menu-box').addClass('hidden');
                    }
                }

                $('#fr-menu-name').val(this.getVal(el.data('name')));
                $('#fr-menu-url').val(this.getVal(el.data('url')));
            };
            ...
            this.saveRelease = function(){
                ...
                $('.menu-item').each(function(index){
                    var currentMenuItem  =  $('.menu-item').eq(index),
                        subMenuItem =  $('.menu-item').eq(index).find('.sub-menu-item');
            ...
            this.getItemJson = function(obj){
                let type = obj.data('type'),url = obj.data('url'),action = obj.data('action'),name = obj.data('name');
                var json = {};
                if(type == 'click'){
                    json =  {type: "click",name: name,key: action,sub_button: []};
                }else if(type == 'view'){
                    // console.log(this.isUrl(url)+'===>'+url);
                    // if(!this.isUrl(url)){
                    //     alert(name +':url地址格式不正确'+url);
                    // }
                    json =  {type: "view",name: name,url: url,sub_button: []};
                }
        ...
        $(document).on('click','.add-sub-item',function(event){
            if($(this).parent().find('.sub-menu-item').length < 5){
                $(this).parent().find('.sub-menu-item').removeClass('current');
                $(this).before(menuObj.getSubItemHtml());
            }else{
                alert('最多只能添加5个子菜单');
            }

            $('#menu-list .menu-item').removeClass('current');
        ...
        $(document).on('click','.menu-item',function(){
            $('.sub-menu-item').removeClass('current');
            $(this).addClass('current').find('.sub-menu-box').show();
            $(this).siblings().removeClass('current').find('.sub-menu-box').hide();

            let o = {
                menuIndex:$(this).index(),
                subMenuIndex:undefined,
                subMenuNum:$(this).addClass('current').find('.sub-menu-item').length
            };

            console.log($(this).data('name'));

            menuObj.setEditArea('menu-item',o,$(this));
        ...
        $(document).on('click','.sub-menu-item',function(event){
            ...
            let o = {
                menuIndex:$(this).parent().parent().parent().index(),
                subMenuIndex:$(this).index(),
                subMenuNum:0
            };

            menuObj.setEditArea('sub-menu-item',o,$(this));
            $('#menu-list .menu-item').removeClass('current');
            event.stopPropagation();    //  阻止事件冒泡
        });
        ...
        $('#fr-menu-name').on('input',function(){
            ...
            if($('.current').hasClass('menu-item')){
                $('.current').data('name',name).find('.title').text(name);
            }else{
                $('.current').data('name',name).find('.sub-title').text(name);
            }
        });

        $('#fr-menu-url').bind('change input',function(){
            $('.current').data('url',$(this).val());
        });
